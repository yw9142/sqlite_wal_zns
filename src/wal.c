...
)
{
  ...
  else
  {
    ...
    assert(pWal->apWiData[iPage] != 0 || rc != SQLITE_OK || (pWal->writeLock == 0 && iPage == 0));
...
static int walFramePage(u32 iFrame)
{
  ...
  assert((iHash == 0 || iFrame > HASHTABLE_NPAGE_ONE) && (iHash >= 1 || iFrame <= HASHTABLE_NPAGE_ONE) && (iHash <= 1 || iFrame > (HASHTABLE_NPAGE_ONE + HASHTABLE_NPAGE)) && (iHash >= 2 || iFrame <= HASHTABLE_NPAGE_ONE + HASHTABLE_NPAGE) && (iHash <= 2 || iFrame > (HASHTABLE_NPAGE_ONE + 2 * HASHTABLE_NPAGE)));
  assert(iHash >= 0);
  return iHash;
}
  ...
  */
  ...
  if (nSize > WAL_HDRSIZE)
  {
    ...
    */
    ...
    if ((magic & 0xFFFFFFFE) != WAL_MAGIC || szPage & (szPage - 1) || szPage > SQLITE_MAX_PAGE_SIZE || szPage < 512)
    {
      goto finished;
    }
...
static int walIteratorInit(Wal *pWal, u32 nBackfill, WalIterator **pp)
{
  ...
  */
  ...
  p = (WalIterator *)sqlite3_malloc64(nByte + sizeof(ht_slot) * (iLast > HASHTABLE_NPAGE ? HASHTABLE_NPAGE : iLast));
  if (!p)
  {
    return SQLITE_NOMEM_BKPT;
  }
...
)
{
  ...
  if (pInfo->nBackfill < pWal->hdr.mxFrame)
  {
    ...
    */
    ...
    if (pIter && (rc = walBusyLock(pWal, xBusy, pBusyArg, WAL_READ_LOCK(0), 1)) == SQLITE_OK)
    {
...
static int walHandleException(Wal *pWal)
{
  if (pWal->exclusiveMode == 0)
  {
    ...
    mUnlock = pWal->lockMask & ~(
                                   (pWal->readLock < 0 ? 0 : (S << WAL_READ_LOCK(pWal->readLock))) | (pWal->writeLock ? (E << WAL_WRITE_LOCK) : 0) | (pWal->ckptLock ? (E << WAL_CKPT_LOCK) : 0)
#ifdef SQLITE_ENABLE_SNAPSHOT
...
static int walAssertLockmask(Wal *pWal)
{
  if (pWal->exclusiveMode == 0)
  {
    static const int S = 1;
    static const int E = (1 << SQLITE_SHM_NLOCK);
    u32 mExpect = ((pWal->readLock < 0 ? 0 : (S << WAL_READ_LOCK(pWal->readLock))) | (pWal->writeLock ? (E << WAL_WRITE_LOCK) : 0) | (pWal->ckptLock ? (E << WAL_CKPT_LOCK) : 0)
#ifdef SQLITE_ENABLE_SNAPSHOT
                   | (pWal->pSnapshot ? (pWal->lockMask & (1 << WAL_CKPT_LOCK)) : 0)
#endif
    );
  ...
  */
  if (badHdr)
  {
    ...
    else
    {
      ...
      if (bWriteLock || SQLITE_OK == (rc = walLockExclusive(pWal, WAL_WRITE_LOCK, 1)))
      {
  ...
  */
  ...
  if (*pCnt > 5)
  {
    ...
    */
    ...
    {
      if (*pCnt & WAL_RETRY_BLOCKED_MASK)
        nDelay = 1;
    }
#endif
  ...
  SEH_INJECT_FAULT;
  {
    ...
    */
    ...
    SEH_INJECT_FAULT;
    walShmBarrier(pWal);
    if (AtomicLoad(pInfo->aReadMark + mxI) != mxReadMark || memcmp((void *)walIndexHdr(pWal), &pWal->hdr, sizeof(WalIndexHdr)))
    {
...
static int walBeginReadTransaction(Wal *pWal, int *pChanged)
{
  ...
  if (rc == SQLITE_OK)
  {
    if (pSnapshot && memcmp(pSnapshot, &pWal->hdr, sizeof(WalIndexHdr)) != 0)
    {
      ...
      ** snapshot.  */
      if (!memcmp(pSnapshot->aSalt, pWal->hdr.aSalt, sizeof(pWal->hdr.aSalt)) && pSnapshot->mxFrame >= pInfo->nBackfillAttempted)
      {
        assert(pWal->readLock > 0);
...
int sqlite3WalSnapshotCheck(Wal *pWal, sqlite3_snapshot *pSnapshot)
{
  ...
  SEH_TRY
  {
    ...
    if (rc == SQLITE_OK)
    {
      ...
      if (memcmp(pNew->aSalt, pWal->hdr.aSalt, sizeof(pWal->hdr.aSalt)) || pNew->mxFrame < walCkptInfo(pWal)->nBackfillAttempted)
      {
        rc = SQLITE_ERROR_SNAPSHOT;
        walUnlockShared(pWal, WAL_CKPT_LOCK);
      }
